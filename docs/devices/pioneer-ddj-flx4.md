...
| Pad FX pad LEDs (both FX layers) | notes `0x10–0x1F` on `0x97` (deck 1) / `0x99` (deck 2), driven by `HeldEcho` feedback rules — bright (`0x7F`) while the pad is physically held, off (`0x00`) otherwise (echoes the pad's own input address via the app's generic held-note tracking). Single-colour, like the hot-cue pads |
