...
- [ ] **Step 2:** Enter Pad FX2 and Beat-Loop; confirm whether the shift addresses (`0x6B`/`0x6D`) give a *distinct* lamp style vs simply lighting the parent. If they do nothing distinct, repoint the 4 shift feedback rules at the parent addresses (`0x1E`/`0x1E`, `0x20`/`0x20`, `0x1B`/`0x1B`, `0x22`/`0x22`) or drop them — **RON data change only, no code change**.
